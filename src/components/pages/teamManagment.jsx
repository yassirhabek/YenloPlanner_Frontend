...
function TeamManagment() {
  ...
  function getAllWerknemers() {
    try{
      ...
      return[{ label: "Wim van der Pluijm", id: 1 },
              { label: "Jane Doe", id: 2 },
              { label: "Joe Doe", id: 3 },
              { label: "Jill Doe", id: 4 }];
    } catch (err) {
  ...
  return (
      ...
      <div>
        ...
        <thead>
          <tr>
            ...
            <th>Naam</th>
            <th>Email</th>
            <th><Button variant='warning' onClick={() => setLgShow(true)}>Add</Button>
            <Modal
              size="lg"
              show={lgShow}
            ...
            >
              ...
              <Modal.Body>
                ...
                <Button className="mt-3" variant="primary" onClick={() => AddWerknemerToTeam()}>Add</Button>
              </Modal.Body>
            </Modal>
            <Button variant='danger' className="mx-1" onClick={deleteTeam}>Delete Team</Button></th>
          </tr>
        </thead>
        <tbody>
          { members === [] ? 
          ...
          :
            members.map((member, index) => 
              <tr key={index}>
                ...
                <td>{member.name}</td>
                <td>{member.email}</td>
                <td><Button variant='danger' onClick={() => deleteMemberFromTeam(member.id)}>Remove</Button></td>
              </tr>
            )}
        </tbody>
      </Table>
    ...
    : <h2>Select a Team to start.</h2>
      ...
      <button className={classes.button} onClick={() => setMgShow(true)}>Create New Team</button> 
      <Modal
        size="lg"
        show={mgShow}
      ...
      >
        ...
        <Modal.Body>
          <TextField id="teamName" label="Team Name" variant="outlined" /><br />
          <Button className="mt-3" variant="primary" onClick={() => CreateTeam()}>Create</Button>
        </Modal.Body>
      </Modal>
    </div>
  );
